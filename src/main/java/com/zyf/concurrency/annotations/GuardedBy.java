...
/**
 * 摘录自原文： @GuardedBy(lock) 表示只有在持有了某个特定的锁时才能访问这个域或方法。 参数 lock表示在访问被标注的域或方法时需要持有的锁。
...
public @interface GuardedBy {
     ...
     * 成员)。
     * • @GuardedBy("fieldName") 表示与fieldName 引用的对象相关联的锁，可以是一个隐式锁式锁（对于不引用一个 Lock 的域），也可以是一个显式锁（对于引用了一个Lock的域）。

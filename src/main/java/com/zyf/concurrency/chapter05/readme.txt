5 基础构建模块
    ...
    5.5 同步工具类
        ...
        同步工具类是一个对象，包含有：
            ...
            * 信号量 semaphore
            * 栅栏 barrier
        平台类库中，存在一些同步工具类类，如果这些不够，可以按照14章的描述，创建一个自己同步工具类

    5.5.1 闭锁 - TestHarness
        闭锁（latch）是一种同步工具类，它可以延迟线程的进度知道线程到达终止（terminal）状态。
    ...
    5.5.2 FutureTask - Preloader, DataLoadException
        FutureTask同样可以作为闭锁。（FutureTask的实现描述了一个抽象的可携带结果的计算[CPJ 4.3.3]）。
        FutureTask的计算是通过Callable实现的，它等价于一个可携带结果的Runnable，并且有三个状态：
                * 等待运行
                * 正在运行
                * 运行完成 完成包括所有计算以任意方式结束，包括正常结束、取消和异常。
                       一旦FutureTask进入完成状态，它会永远停止在这个状态上。
    ...
    5.5.3 信号量 - BoundedHashSet
        ...
        概念：
            * 许可-permit 指向操作时可以先获得许可，并在使用后释放许可
            * 获得-acquire 如果没有许可，则acquire将阻塞直到有许可（或者知道被中断或者操作超时）
            * 释放-release 该方法返回一个许可给信号量

        可以将acquire视为是消费一个许可，而release操作是创建一个许可，Semaphore并不受限于它在创建时的初始许可数量。Semaphore不会将许可与线程关联起来，因此在一个线程中获得的许可可以在另一个线程中释放
        二值信号量可以用作互斥体（mutex），并具备-不可重入-的加锁语义：谁拥有这个唯一许可，谁就拥有了互斥锁。
        场景：可用于实现资源池，如数据库连接
            请求一个固定长度的资源池，当池为空时，我们希望看到的是阻塞而不是失败。
            在12章的有界缓冲区将使用这个技术(构造对象池可用BlockingQueue来保存池的资源)

        关键字
            二值信号量
                即初始值为1的Semaphore

    5.5.4 栅栏 - CellularAutomata
...
第一部分小节
    ...
    * 不可变对象一定是线程安全的
        不可变对象能极大地降低并发编程的复杂性。它们更为简单而且安全，可以任意共享二无须使用加锁或保护性复制等机制。
    * 封装有助于管理复杂性
        在编写线程安全的程序时，虽然可以将所有数据都保存在全局变量汇总，但为什么这样做？将数据封装在对象中，更易于维护不变性条件：将同步机制封装在对象中，更易于遵循同步策略。
    * 用锁保护每个可变变量
